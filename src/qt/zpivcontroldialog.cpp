    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zDEXR.";
            else if (!mint.isSeedCorrect)
                strReason = "The zDEXR seed used to mint this zDEXR is not the same as currently hold in the wallet";
            else
