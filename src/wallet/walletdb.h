...
public:
    ...
    bool WriteZDEXRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDEXRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDEXRSeed_deprecated(uint256& seed);
    bool EraseZDEXRSeed();
    bool EraseZDEXRSeed_deprecated();

    bool WriteZDEXRCount(const uint32_t& nCount);
    bool ReadZDEXRCount(uint32_t& nCount);
