...
CzDEXRTracker::CzDEXRTracker(std::string strWalletFile)
{
...
CzDEXRTracker::~CzDEXRTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDEXRTracker::Init()
{
...
bool CzDEXRTracker::Archive(CMintMeta& meta)
{
...
bool CzDEXRTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDEXRTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDEXRTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDEXRTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzDEXRTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzDEXRTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzDEXRTracker::GetSerialHashes()
{
...
CAmount CzDEXRTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDEXRTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDEXRTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDEXRTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDEXRTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDEXRTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDEXRTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDEXRTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDEXRTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDEXRTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDEXRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzDEXRWallet* zDEXRWallet)
{
    bool iszDEXRWalletInitialized = (NULL != zDEXRWallet);
    CMintMeta meta;
    ...
    if (! iszDEXRWalletInitialized)
        zDEXRWallet = new CzDEXRWallet(strWalletFile);
    meta.isSeedCorrect = zDEXRWallet->CheckSeed(dMint);
    if (! iszDEXRWalletInitialized)
        delete zDEXRWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzDEXRTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDEXRTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDEXRTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDEXRTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDEXRTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDEXRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzDEXRWallet* zDEXRWallet = new CzDEXRWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zDEXRWallet);
        }
        delete zDEXRWallet;
